 ...
 */
class Migration_Seed_student_users extends CI_Migration {
	...
	public function up()
	{
		if ($this->seed)
		{
			...
			$student_role = $this->db->where('role_key', 'student')->get('roles')->row();

			$teacher_user_schools = $this->db->join('user_roles', 'user_schools.user_id = user_roles.user_id')
			...
			foreach ($teacher_user_schools as $teacher_user_school)
			{
				$user_roles = array();
				$user_schools = array();
				...
				$users_count = $faker->numberBetween(20 * $magnitude, 40 * $magnitude);
				for ($i = 0; $i < $users_count; $i++)
				{
					...
					$user_schools[] = array(
						'user_id'   => $user_id,
						'school_id' => $teacher_user_school->school_id
					);
				}
